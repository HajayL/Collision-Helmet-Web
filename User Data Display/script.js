    ...
    if(resp.success && helmets.length > 0){
      ...
      for(let h = 0; h < helmets.length; h++){
        for(let i = data[helmets[h]].length - 1; i >= 0; i--){
          currentData[helmets[h]].accelX[currentData[helmets[h]].accelX.length] = (parseInt(data[helmets[h]][i].accel.x)/1000).toFixed(2);
          currentData[helmets[h]].accelY[currentData[helmets[h]].accelY.length] = -(parseInt(data[helmets[h]][i].accel.y)/1000).toFixed(2);
          currentData[helmets[h]].accelZ[currentData[helmets[h]].accelZ.length] = (parseInt(data[helmets[h]][i].accel.z)/1000).toFixed(2);
          currentData[helmets[h]].gyroX[currentData[helmets[h]].gyroX.length] = (parseInt(data[helmets[h]][i].gyro.x)/1000).toFixed(2);
          currentData[helmets[h]].gyroY[currentData[helmets[h]].gyroY.length] = (parseInt(data[helmets[h]][i].gyro.y)/1000).toFixed(2);
          currentData[helmets[h]].gyroZ[currentData[helmets[h]].gyroZ.length] = (parseInt(data[helmets[h]][i].gyro.z)/1000).toFixed(2);
